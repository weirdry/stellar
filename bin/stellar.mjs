#!/usr/bin/env node
// Generated by just build-runner. Edit bin/stellar.ts and lib/, then regenerate.
...
var init_package = __esm({
  "package.json"() {
    package_default = { version: "0.1.3-dev.0" };
  }
});
...
function loadSchema(url) {
  ...
  if (isObject(value) && (value["$async"] === void 0 || value["$async"] === false))
    return value;
  throw new TypeError("Schema must be an object or boolean.");
}
function schemaProperty(error) {
  const params = error.params;
  const name = property(params, "missingProperty") ?? property(params, "additionalProperty");
  return typeof name === "string" ? name : void 0;
}
function required(value, invariant) {
  if (value === void 0) throw new Error(invariant);
  return value;
}
...
function validManifest(parsed) {
  if (!isObject(parsed) || parsed["version"] !== version) return false;
  const files = parsed["files"];
  return isObject(files) && !Array.isArray(files) && Object.keys(files).length === runtimeFiles.length && runtimeFiles.every((path) => {
    const hash = files[path];
    return Object.hasOwn(files, path) && typeof hash === "string" && /^[a-f0-9]{64}$/.test(hash);
  });
}
  ...
  const read = async (path) => {
    try {
      ...
      add(
        path,
        "fail",
        property(error, "code") === "ENOENT" ? "Required file is missing." : "Required file cannot be read.",
        remedy
      );
      return null;
    }
...
var require_limit = __commonJS({
  "node_modules/.pnpm/ajv-formats@3.0.1_ajv@8.20.0/node_modules/ajv-formats/dist/limit.js"(exports) {
    ...
    var formatLimitPlugin = (ajv4) => {
      ajv4.addKeyword(exports.formatLimitDefinition);
      return ajv4;
    };
...
var require_dist = __commonJS({
  "node_modules/.pnpm/ajv-formats@3.0.1_ajv@8.20.0/node_modules/ajv-formats/dist/index.js"(exports, module) {
    ...
    var formatsPlugin = (ajv4, opts = { keywords: true }) => {
      if (Array.isArray(opts)) {
        addFormats4(ajv4, opts, formats_1.fullFormats, fullName);
        return ajv4;
      }
      const [formats, exportName] = opts.mode === "fast" ? [formats_1.fastFormats, fastName] : [formats_1.fullFormats, fullName];
      const list = opts.formats || formats_1.formatNames;
      addFormats4(ajv4, list, formats, exportName);
      if (opts.keywords)
        (0, limit_1.default)(ajv4);
      return ajv4;
    };
    ...
    function addFormats4(ajv4, list, fs, exportName) {
      var _a;
      var _b;
      (_a = (_b = ajv4.opts.code).formats) !== null && _a !== void 0 ? _a : _b.formats = (0, codegen_1._)`require("ajv-formats/dist/formats").${exportName}`;
      for (const f of list)
        ajv4.addFormat(f, fs[f]);
    }
...
function safeAttachmentURL(value) {
  ...
  try {
    ...
    return !/[:\\?#\s]/.test(path) && ![...path].some((char) => char.charCodeAt(0) < 32) && !path.startsWith("/") && path.split("/").every((part) => part && part !== "." && part !== "..");
  } catch {
    return false;
  }
}
function evaluateWorkMap(data) {
  const diagnostics = [];
  const add = (code, path, message, fix) => diagnostics.push({ code, path, message, fix });
  if (!checkShape(data)) {
    for (const error of checkShape.errors ?? []) {
      const property2 = schemaProperty(error);
      add(
        "schema",
        (error.instancePath || "") + (property2 ? "/" + property2 : "") || "/",
        error.message ?? "",
        "Match the field to schemas/work-map.schema.json; do not fabricate missing source facts."
      );
    }
    return { valid: false, diagnostics, map: void 0 };
  }
  ...
  const namespaces = /* @__PURE__ */ new Set(), nativeIds = /* @__PURE__ */ new Set();
  data.sources.forEach((source, n) => {
    if (source.provider === "github" && (source.namespace !== source.namespace.toLowerCase() || !/^[^/\s]+\/[^/\s]+\/[^/\s]+$/.test(source.namespace)))
      add(
        "source-namespace",
        `/sources/${n}/namespace`,
  ...
  data.issues.forEach((issue, n) => {
    ...
    if (issue.classificationEvidence && (!issue.classification || issue.detail !== "unqueried"))
      add(
  ...
  data.relations.forEach((edge, n) => {
    ...
    const ends = edge.kind === "related" ? [edge.source, edge.target].sort() : [edge.source, edge.target];
    const identity2 = [edge.kind, ...ends].join("|");
...
function validateWorkMap(data) {
  const { valid, diagnostics } = evaluateWorkMap(data);
  return { valid, diagnostics };
}
function assertWorkMap(data, allowPending = false) {
  const result = evaluateWorkMap(data);
  const diagnostics = allowPending ? result.diagnostics.filter(
...
async function renderWorkMap(input) {
...
function normalizeCapture(capture) {
  ...
  const aliases = /* @__PURE__ */ new Map(), records = [];
  ...
  for (const [n, entry] of capture.records.entries()) {
    ...
    if (source.provider === "github") {
      ...
      if (typeof raw["number"] !== "number" || !Number.isSafeInteger(raw["number"]) || raw["number"] < 1 || raw["state"] !== "open" && raw["state"] !== "closed")
        fail(
          path,
  ...
  for (const { entry, source, path, relations } of records) {
    for (const observation of [
      ...
      const native = observedSource.provider === "linear" ? ref["uuid"] : ref["node_id"];
  ...
  const identify = (source, ref, path) => {
    const native = aliases.get(aliasKey(source, nativeOf(source, ref))) || nativeOf(source, ref);
    if (!nonblank(native))
    ...
    const identity2 = required(
      identities.get(id),
      "Identity was inserted before use."
    );
    identity2.refs.push({ ref, path });
    return identity2;
  };
  ...
  for (const { entry, source, path, identity: identity2 } of records) {
    const raw = entry.data, { id, native } = required(identity2, "Record identity has been resolved.");
    if (fullIds.has(id))
      fail(
        path,
    ...
    if (source.provider === "linear") {
      ...
      copy(
        "priority",
        typeof raw["priority"] === "object" && raw["priority"] !== null ? property(raw["priority"], "name") : raw["priority"]
      );
    } else {
      ...
      copy(
        "assignee",
        (isArray(raw["assignees"]) ? raw["assignees"].map((assignee, index) => {
          if (!assignee || !isObject(assignee) || Array.isArray(assignee) || !nonblank(assignee["login"]))
            fail(
      ...
      copy(
        "labels",
        isArray(raw["labels"]) ? raw["labels"].map((label, index) => {
          const name = typeof label === "string" ? label : label && isObject(label) && !Array.isArray(label) ? label["name"] : void 0;
          if (!nonblank(name))
            fail(
      ...
      copy(
        "completedAt",
        raw["state_reason"] === "completed" ? raw["closed_at"] : void 0,
        "closed_at"
      );
    }
  ...
  for (const { id, source, native, refs } of identities.values()) {
    ...
    const identifier = firstText(displays.filter((value) => value !== native)) || native;
    const firstRef = (field) => refs.flatMap((entry) => {
  ...
  for (const { identity: identity2, relations } of records)
    for (const { kind, reverse, identity: other, path } of relations)
      addEdge(
        kind,
        reverse ? required(other, "Endpoint identity resolved.").id : required(identity2, "Record identity resolved.").id,
        reverse ? required(identity2, "Record identity resolved.").id : required(other, "Endpoint identity resolved.").id,
        path
      );
  try {
...
function collectRelations(source, entry, path) {
  const raw = entry.data, observations = [];
  const complete = source.coverage.relations === "complete" && entry.scope === "assigned";
  const links = entry.links || {};
  const list = (value, name, kind, reverse = false) => {
    if (value === void 0 && !complete) return;
    if (!isArray(value))
      fail(
        path + "/" + name,
...
function endpointReference(ref, path) {
  if (!isObject(ref))
    fail(
      path,
...
function endpointSource(sources, source, ref, path) {
  ...
  if (source.provider === "github") {
    if (typeof ref["number"] !== "number" || !Number.isSafeInteger(ref["number"]) || ref["number"] < 1 || ref["pull_request"])
      fail(
        path,
    ...
    targetSource = sources.find(
      (candidate) => candidate.provider === "github" && githubBelongsTo(candidate, ref)
    );
    if (!targetSource)
      fail(
        path,
...
function assertGithubUrl(raw, path) {
  if (!nonblank(raw["html_url"]) || !/^https?:\/\//.test(raw["html_url"]) || !URL.canParse(raw["html_url"]))
    fail(
      path + "/html_url",
...
function githubBelongsTo(source, raw) {
  try {
    ...
    return `${url.host}/${url.pathname.split("/").slice(1, 3).join("/")}`.toLowerCase() === source.namespace.toLowerCase();
  } catch {
    return false;
  }
}
function normalizeStatus(source, raw) {
  ...
  const type = raw["state"] === "open" ? "unstarted" : raw["state_reason"] === "completed" ? "completed" : raw["state_reason"] === "not_planned" ? "canceled" : raw["state_reason"] === "duplicate" ? "duplicate" : "unknown";
  return {
    label: raw["state_reason"] ? `${sourceText(raw["state"])} \xB7 ${sourceText(raw["state_reason"])}` : raw["state"],
    type
  };
}
...
var import_ajv2, import_ajv_formats2, workMapSchema, captureSchema, ajv2, checkCapture, nonblank, keyOf, nativeOf, displayOf, sourceText;
var init_normalize = __esm({
  "lib/normalize.ts"() {
    ...
    init_validate();
    workMapSchema = loadSchema(
      new URL("../schemas/work-map.schema.json", import.meta.url)
    );
    captureSchema = loadSchema(
      new URL("../schemas/capture.schema.json", import.meta.url)
    );
    ajv2 = new import_ajv2.Ajv({ strict: true, allErrors: true });
    import_ajv_formats2.default.default(ajv2);
    ...
    displayOf = (source, raw) => source.provider === "linear" ? raw["id"] : "#" + String(raw["number"]);
...
function fail2(path, message, fix) {
...
function shape(check, value, input) {
  if (!check(value)) {
    const alternatives = input === "choices" ? (check.errors ?? []).filter(
      (error) => error.keyword === "anyOf" && ["#/anyOf", "#/properties/issues/items/anyOf"].includes(
        error.schemaPath
      )
    ) : [];
    const errors = (check.errors ?? []).filter(
      (error) => !alternatives.some(
        (alternative) => error.schemaPath.startsWith(alternative.schemaPath + "/") && (error.instancePath === alternative.instancePath || error.instancePath.startsWith(alternative.instancePath + "/"))
      )
    );
    throw new WorkMapError(
      errors.map((error) => {
        ...
        return {
          ...
          input,
          path: (error.instancePath || "") + (property2 !== void 0 ? "/" + property2.replace(/~/g, "~0").replace(/\//g, "~1") : "") || "/",
...
function assertState(state) {
  ...
  const diagnostics = validateWorkMap(state.map).diagnostics.filter((d) => d.code !== "missing-classification").map((d) => ({ ...d, path: "/map" + d.path }));
...
function rememberIssue(map, issue) {
  return {
    ...
    ...issue.classification ? { classification: clone(issue.classification) } : {},
...
function classifyDraft(input, choices) {
  const map = assertWorkMap(input, true);
  assertRunReferences(map);
  const memory = map.issues.map((issue) => ({
    ...rememberIssue(map, issue),
    ...issue.scope === "assigned" && !issue.classification ? { reviewReason: "new-issue" } : {}
  }));
...
function refreshState(input, capture) {
  ...
  for (const issue of map.issues) {
    const id = identity(map, issue), stable = key(id);
    let saved = memory.get(stable);
    observed.add(stable);
    if (!saved) {
      changes.added.push(issue.id);
      const uncertain = previous.memory.some(
        (entry) => entry.provider === id.provider && entry.namespace === id.namespace && entry.identifier === issue.identifier
      );
    ...
    if (!prior) changes.returned.push(issue.id);
      ...
      const fields = [
        .../* @__PURE__ */ new Set([...Object.keys(prior), ...Object.keys(issue)])
      ].filter(
        (field) => ![
          "id",
          "sourceId",
          ...
          "targets"
        ].includes(field) && !equal(property(prior, field), property(issue, field))
      ).sort();
      if (fields.length) changes.updated.push({ issueId: issue.id, fields });
    }
    const changedPurpose = issue.detail === "full" && !equal(saved.evidence && evidence(saved.evidence), evidence(issue));
  ...
  changes.notObserved = [...memory.values()].filter((entry) => !observed.has(key(entry))).map(({ provider, namespace, nativeId }) => ({
    provider,
    namespace,
    nativeId
  }));
...
function applyChoices(input, choices, actor) {
  ...
  function updateGroups(field, additions, current) {
    ...
    for (const [n, item] of additions.entries()) {
      ...
      seen2.add(item.id);
      if (field === "categories" && !map.domains.some((domain) => domain.id === property(item, "domain")))
        fail2(
          `/categories/${n}/domain`,
...
async function writeRun(input, directory) {
  ...
  try {
    ...
    const exists = property(error, "code") === "EEXIST" && parentReady && !ownsDirectory;
    const failure = new WorkMapError([
      {
        code: "run-output",
        path: "/run",
        message: exists ? "The output run path already exists." : ["EACCES", "EPERM", "EROFS"].some(
          (code) => code === property(error, "code")
        ) ? "The output run location is not writable." : property(error, "code") === "ENOTDIR" || property(error, "code") === "EEXIST" && !parentReady ? "An output parent path is not a directory." : "The output run could not be written.",
        fix: exists ? "Choose a fresh unused run directory; keep existing inputs and reports." : "Check the output parent directory, write permissions and available space; retry with a fresh unused run directory and keep earlier runs.",
        ...cleanupFailed ? {
          cleanup: "Some newly created output files may remain. Inspect the failed run directory; do not use it as a saved state."
        } : {}
      }
    ]);
    failure.cause = error;
...
var import_ajv3, import_ajv_formats3, schema2, ajv3, checkState, checkChoices, clone, key, identity, evidence, blankChanges, currentReviews;
var init_continuity = __esm({
  "lib/continuity.ts"() {
    ...
    init_validate();
    init_normalize();
    schema2 = (name) => loadSchema(new URL(`../schemas/${name}.schema.json`, import.meta.url));
    ajv3 = new import_ajv3.Ajv({ strict: true, allErrors: true });
    import_ajv_formats3.default.default(ajv3);
...
async function verifyRun({
  ...
  const state = inputState === void 0 ? void 0 : withRole("state", () => assertState(inputState));
  const checks = {};
  const diagnostics = [];
  ...
  const slots = [
    ...htmlBytes.toString("utf8").matchAll(
...
async function verifyRunFiles(capture, map, html, state) {
  return verifyRun({
...
async function readReadingMap(path) {
  try {
...
function bodyBlocks(text) {
  ...
  for (const line of lines) {
    if (fence && current) {
      ...
      if (close.length >= fence.length && [...close].every((c) => c === fence?.[0])) {
        fence = void 0;
        current = void 0;
      }
      continue;
    ...
    const setext = /^ {0,3}(?:=+|-+)\s*$/.test(line) && current?.kind === "paragraph" && !blank;
    if (setext && current) {
    ...
    const kind = opening ? "code" : /^ {0,3}#{1,6}(?:\s|$)/.test(line) ? "heading" : /^\s*(?:[-+*]|\d+[.)])\s/.test(line) ? "list" : /^\s*>/.test(line) ? "quote" : "paragraph";
    const continuation = current?.kind === "list" && /^\s+\S/.test(line);
    if (!current || blank || opening || kind === "heading" || kind !== current.kind && !continuation) {
      current = { kind, text: "" };
      blocks.push(current);
    }
    current.text += line;
    ...
    else if (kind === "heading") current = void 0;
  }
  let position = 0;
...
function inspectMap(input, selector, offset = 0) {
  const map = assertWorkMap(input, true);
...
function searchIssue(input, selector, query, offset = 0) {
  ...
  for (const match of body.matchAll(new RegExp(RegExp.escape(query), "gu"))) {
    ...
    while (required(blocks[block], "Search match belongs to a body block.").end <= position)
      block++;
    if (total >= offset && matches.length < PAGE)
      matches.push({
        block,
        offset: position - required(blocks[block], "Search match belongs to a body block.").start,
...
async function retainResponse(input, output) {
  ...
  try {
    ...
    let fix = "Choose a new writable file path. Existing responses are never replaced.";
    if (property(error, "code") === "ENOTDIR" || creatingParents && property(error, "code") === "EEXIST") {
    ...
    } else if (property(error, "code") === "EEXIST" || property(error, "code") === "EISDIR") {
    ...
    } else if (property(error, "code") === "EACCES" || property(error, "code") === "EPERM") {
      code = "response-permission";
      message = "Response destination is not writable.";
      fix = "Check destination-directory permissions or choose a writable directory; keep existing responses.";
    }
...
async function runCommand(command2, args2) {
  const input = required(args2[0], "CLI argument count was validated.");
  const [, output, ...extra] = args2;
  ...
  try {
    if (command2 === "retain-response") {
      ...
      const draft = await readWorkMap(input), choices = await readWorkMap(second(), "choices");
      console.log(
    ...
    if (error instanceof WorkMapError)
      ...
      console.error(
        error instanceof SyntaxError ? "Input is not valid JSON." : property(error, "message")
      );
    process.exitCode = 1;
  }
}
...
function errorSummary(error) {
  ...
  const code = typeof errorCode === "string" && ["ENOENT", "EACCES", "EPERM", "ERR_MODULE_NOT_FOUND"].includes(errorCode) ? ` (${errorCode})` : "";
...
function commandInfo(name) {
  return name !== void 0 && Object.hasOwn(commands, name) ? commands[name] : void 0;
}
