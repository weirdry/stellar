#!/usr/bin/env node
// Generated by just build-runner. Edit bin/stellar.js and lib/, then regenerate.
...
function safeAttachmentURL(value) {
  ...
  try {
    ...
    return !/[:\\?#\s]/.test(path) && ![...path].some((char) => char.codePointAt(0) < 32) && !path.startsWith("/") && path.split("/").every((part) => part && part !== "." && part !== "..");
  } catch {
    return false;
  }
}
...
function normalizeCapture(capture) {
  ...
  for (const [n, entry] of capture.records.entries()) {
    ...
    if (source.provider === "github") {
      ...
      if (!Number.isSafeInteger(raw.number) || raw.number < 1 || !["open", "closed"].includes(raw.state))
        fail(
          path,
  ...
  const identify = (source, ref, path) => {
    ...
    const identity2 = identities.get(id);
  ...
  for (const { entry, source, path, identity: identity2 } of records) {
    const raw = entry.data, { id, native } = identity2;
    if (fullIds.has(id))
      fail(
        path,
    ...
    if (source.provider === "linear") {
      ...
      copy(
        "priority",
        typeof raw.priority === "object" && raw.priority !== null ? raw.priority.name : raw.priority
      );
    } else {
      ...
      copy(
        "assignee",
        raw.assignees?.map((assignee, index) => {
          if (!assignee || typeof assignee !== "object" || Array.isArray(assignee) || !nonblank(assignee.login))
            fail(
      ...
      copy(
        ...
        raw.labels?.map((label, index) => {
          const name = typeof label === "string" ? label : label && typeof label === "object" && !Array.isArray(label) ? label.name : void 0;
          if (!nonblank(name))
            fail(
  ...
  for (const { id, source, native, refs } of identities.values()) {
    ...
    const firstRef = (field) => refs.filter(({ ref }) => nonblank(ref[field])).toSorted(
      (a, b) => a.ref[field] < b.ref[field] ? -1 : a.ref[field] > b.ref[field] ? 1 : 0
    )[0];
  ...
  const addEdge = (kind, source, target, path) => {
    const ends = kind === "related" ? [source, target].sort() : [source, target];
  ...
  for (const { identity: identity2, relations } of records)
    for (const { kind, reverse, identity: other, path } of relations)
      addEdge(
        kind,
        reverse ? other.id : identity2.id,
        reverse ? identity2.id : other.id,
        path
      );
  const diagnostics = validateWorkMap(map).diagnostics.filter((d) => d.code !== "missing-classification").map((d) => ({ ...d, path: capturePath(d.path) }));
...
function normalizeStatus(source, raw) {
  if (source.provider === "linear")
    return {
      ...
      type: [
        ...
        "canceled",
        "duplicate"
      ].includes(raw.statusType) ? raw.statusType : "unknown"
    };
  ...
  return {
    label: raw.state_reason ? `${raw.state} \xB7 ${raw.state_reason}` : raw.state,
    type
  };
}
...
function shape(check2, value) {
  if (!check2(value)) {
    ...
    const errors = check2.errors.filter(
...
function classifyDraft(map, choices) {
...
function refreshState(previous, capture) {
...
function applyChoices(previous, choices, actor) {
  assertState(previous);
  ...
  for (const field of ["domains", "categories"]) {
    ...
    for (const [n, item] of (choices[field] || []).entries()) {
      ...
      seen2.add(item.id);
      if (field === "categories" && !map.domains.some((domain) => domain.id === item.domain))
        fail2(
          `/categories/${n}/domain`,
  ...
  for (const [n, choice] of (choices.issues || []).entries()) {
    ...
    if (choice.classification) {
      ...
      if (actor === "agent" && saved.classification?.origin === "user") {
        if (!equal(choice.classification, {
...
async function writeRun(state, directory) {
  assertState(state);
  const files = {
    ...
    const failure = new WorkMapError([
      {
        code: "run-output",
        path: "/run",
        message: exists ? "The output run path already exists." : ["EACCES", "EPERM", "EROFS"].includes(error.code) ? "The output run location is not writable." : error.code === "ENOTDIR" || error.code === "EEXIST" && !parentReady ? "An output parent path is not a directory." : "The output run could not be written.",
...
async function verifyRun({ capture, map, html, state }) {
...
function inspectMap(map, selector, offset = 0) {
  check(map);
...
function readIssue(map, selector, block, offset = 0) {
  check(map);
...
function searchIssue(map, selector, query, offset = 0) {
  check(map);
  offset = number(offset, "/offset");
...
async function retainResponse(input, output) {
  ...
  try {
    ...
    if (error.code === "ENOTDIR" || creatingParents && error.code === "EEXIST") {
