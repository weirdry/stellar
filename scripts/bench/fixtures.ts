import type { Capture, Issue } from '../../lib/contracts.ts';
...
const sources: Capture['sources'] = (['linear', 'github'] as const).map(
  (provider) => ({
    id: `${provider}-bench`,
    provider,
...
const category = (issue: Issue) =>
  `group-${Number(required(issue.nativeId.match(/\d+$/))[0]) % 32}`;
const load = (file: string): unknown => JSON.parse(readFileSync(file, 'utf8'));
const loadState = (file: string) => {
  return assertState(load(file));
};
const save = (file: string, value: unknown) =>
  writeFileSync(file, JSON.stringify(value, null, 2) + '\n');

function capture(n: number, mode: string): Capture {
  ...
  for (const parity of [0, 1]) {
    ...
    for (const [j, i] of group.entries()) {
      ...
      const blocks = [
        ref(required(group[(j + 1) % group.length])),
        ref(required(group[(j + 17) % group.length])),
      ];
      const parent = j > 0 ? required(group[Math.floor((j - 1) / 2)]) : null;
      if (parity === 0) {
        records.push({
...
if (!input || !output)
  throw new Error('Expected operation, input, and output.');
if (operation === 'generate') {
  ...
  const map = assertWorkMap(load(input), true);
  map.domains = [
    {
      id: 'benchmark',
  ...
  for (const issue of map.issues) {
    issue.classification = {
      ...
      origin:
        Number(required(issue.nativeId.match(/\d+$/))[0]) % 4 === 0
          ? 'user'
          : 'agent',
    };
  ...
  const prior = loadState(input),
    next = loadState(output);
  ...
  for (const issue of users) {
    assert.deepEqual(
      issue.classification,
      required(old.get(issue.id)).classification,
    );
    assert.deepEqual(issue.targets, required(old.get(issue.id)).targets);
  }
