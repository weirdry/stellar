...
fn status(s: &V, r: &V) -> Result<V, String> {
    ...
    if linear(s) {
        ...
        if truth(&r["state_reason"]) {
            let reason = r["state_reason"]
                .as_str()
                .ok_or("unsupported status coercion")?;
...
fn run(input: V) -> Result<V, String> {
    ...
    Ok(json!({"issues":issues,"relations":relations}))
}
fn main() {
    let mut bytes = vec![];
    let result = io::stdin()
    ...
    match result {
        Ok(v) => {
            if serde_json::to_writer(io::stdout().lock(), &v).is_err() {
                std::process::exit(1)
            }
        }
        Err(_) => {
            eprintln!("worker declined input");
            std::process::exit(1)
        }
    }
}
