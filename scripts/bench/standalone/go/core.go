// Experimental normalization kernel; cli.go owns capture parsing, validation and storage.
package main
...
// Output objects use native maps; equality is semantic, with array order preserved.
...
func status(s Source, r V) Obj {
	...
	if s.Provider == "linear" {
		...
		if truth(r["state_reason"]) {
			reason := jsString(r["state_reason"])
