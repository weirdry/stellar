...
function setDefaultTheme(html: string) {
...
function generatorHtml(html: string) {
...
function exportSvg(html: string) {
  ...
  const themeBlocks = [
    required(required(styles[0])[1]).match(
  ...
  const colors = Object.fromEntries(
    themeBlocks.flatMap((block) =>
      [...required(block).matchAll(/(--[\w-]+):\s*([^;]+);/g)].map((match) => [
        required(match[1]),
        required(match[2]).trim(),
      ]),
    ),
  );
  const resolveColors = (text: string) =>
    text.replace(
      /var\((--[\w-]+)\)/g,
      (match: string, name: string) => colors[name] ?? match,
    );
  const svg = resolveColors(required(required(svgs[0])[0]));
  const viewBox = svg.match(/viewBox="0 0 ([\d.]+) ([\d.]+)"/);
  if (!viewBox) throw new Error('Missing finite SVG viewBox.');
  const [, width, height] = viewBox;
  const css = resolveColors(required(required(styles[0])[1]).trimEnd());
  if (css.includes(']]>'))
...
try {
  if (!['build', 'check'].includes(mode))
    throw new Error('Usage: diagrams.ts [build|check]');
  ...
  if (mode === 'build') {
    ...
    for (const name of names) {
      const type = string(
        record(JSON.parse(read(`${name}.json`)))['diagram_type'],
      );
      const result = record(
        JSON.parse(
          execFileSync(
      ...
      if (
        ...
        record(result['specification'])['sha256'] !==
          digest(read(`${name}.json`)) ||
        record(result['artifact'])['sha256'] !== digest(read(`${name}.html`))
      ) {
        throw new Error(
      ...
      if (
        !checked['ok'] ||
        array(checked['checks']).length !== 9 ||
        array(checked['checks']).some((check) => !record(check)['ok']) ||
        record(record(checked['composition'])['summary'])['errors'] !== 0 ||
        record(record(checked['composition'])['summary'])['warnings'] !== 0
      ) {
  ...
  if (mode === 'check') {
    ...
    const previous: unknown = JSON.parse(readFileSync(manifestPath, 'utf8'));
