...
const preview = (text) => characters(text).slice(0, PREVIEW).join('');

function fail(path, message, repair) {
  throw new WorkMapError([{ path, code: 'reading', message, repair }]);
}
...
function metadata(issue) {
  ...
  return {
    ...
    detail: issue.detail,
    status: issue.status,
    descriptionPresent: typeof issue.description === 'string',
...
export function inspectMap(map, selector, offset = 0) {
  ...
  const blocks = bodyBlocks(issue.description ?? '').map(
    ({ text, ...block }) => ({
      ...
      previewTruncated: characters(text).length > PREVIEW,
    }),
  );
  return {
    kind: 'body-index',
...
export function searchIssue(map, selector, query, offset = 0) {
  ...
  for (const block of bodyBlocks(issue.description ?? '')) {
    let at = block.text.indexOf(query);
    let from = 0;
    let position = 0;
    while (at !== -1) {
      position += characters(block.text.slice(from, at)).length;
      if (total >= offset && matches.length < PAGE)
        matches.push({
          block: block.block,
          offset: position,
          preview: preview(block.text.slice(at)),
        });
      total++;
      position += characters(query).length;
      from = at + query.length;
      at = block.text.indexOf(query, from);
    }
  }
  if (offset > total)
    fail(
