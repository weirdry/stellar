...
export function normalizeCapture(capture) {
  ...
  const bind = (source, alias, id, path) => {
    ...
    if (aliases.has(key) && aliases.get(key) !== id)
      fail(
        path,
        'Native identifiers resolve to conflicting issues.',
  ...
  for (const [n, entry] of capture.records.entries()) {
    ...
    const id = keyOf(source.id, nativeOf(source, raw));
    if (entries.has(id))
      fail(
        path,
    ...
    bind(source, nativeOf(source, raw), id, path);
    bind(source, displayOf(source, raw), id, path);
    const issue = {
      id,
      sourceId: source.id,
      nativeId: nativeOf(source, raw),
  ...
  const resolveEndpoint = (source, ref, path) => {
    ...
    if (source.provider === 'github') {
      ...
      targetSource = capture.sources.find(
        (candidate) =>
    ...
    const native = nativeOf(targetSource, ref);
    if (!nonblank(native))
      fail(
        path,
    ...
    const nativeMatch = aliases.get(aliasKey(targetSource, native));
    const identifierMatch = aliases.get(
      aliasKey(targetSource, displayOf(targetSource, ref)),
    );
    if (nativeMatch && identifierMatch && nativeMatch !== identifierMatch)
      fail(
        path,
        'Relation native identity and identifier disagree.',
        'Resolve the conflicting endpoint against the source before normalizing.',
      );
    const existing = nativeMatch || identifierMatch;
    if (existing) return existing;
    const id = keyOf(targetSource.id, native),
      identifier = displayOf(targetSource, ref);
    const context = {
      id,
      sourceId: targetSource.id,
      nativeId: native,
      identifier,
      title: nonblank(ref.title) ? ref.title : identifier,
      scope: 'context',
      detail: 'unqueried',
      status: {
        type: 'unknown',
    ...
    const url = targetSource.provider === 'linear' ? ref.url : ref.html_url;
    if (url) context.url = url;
    map.issues.push(context);
    bind(targetSource, native, id, path);
    bind(targetSource, identifier, id, path);
    return id;
  };
  const edges = new Set();
  ...
  for (const [id, entry] of entries) {
    const source = sources.get(entry.sourceId),
      raw = entry.data,
      path = `/records/${capture.records.indexOf(entry)}`;
    const complete =
    ...
    const list = (value, name, kind, reverse = false) => {
      ...
      for (const ref of value) {
        const other = resolveEndpoint(source, ref, path + '/' + name);
        addEdge(kind, reverse ? other : id, reverse ? id : other);
      }
    };
    if (source.provider === 'linear') {
      ...
      if (rel.duplicateOf)
        addEdge(
          'duplicate',
          id,
          resolveEndpoint(source, rel.duplicateOf, path),
        );
      if (raw.parentId)
        addEdge(
          'parent',
          resolveEndpoint(source, { id: raw.parentId }, path),
          id,
        );
      ...
      if (links.parent)
        addEdge('parent', resolveEndpoint(source, links.parent, path), id);
      list(links.children, 'links/children', 'parent');
