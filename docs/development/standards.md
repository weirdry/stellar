...
The foundation has shell support scripts and Markdown, with no product runtime
or native application manifest. Only common tooling requirements currently
apply. A Node, Python, workspace, or publishing profile must be selected from
actual source and delivery needs when that surface is added. No dummy manifest,
native-root map, release-unit map, central agent, or reusable conformance
workflow is needed.
...
distribution remain **Open**. Revisit runtime/dependency obligations with the
first implementation and publishing obligations with the actual artifact.
