...
import { element as $, elements as $$, required, data } from './dom.ts';
import type { TextLabel } from './dom.ts';
// renderWorkMap validates the canonical schema before embedding this payload.
// This assertion describes that producer boundary; display fields live separately.
const input = JSON.parse(required($('#data').textContent)) as WorkMap;
...
// The renderer embeds the repository-owned locale catalog.
const messages = JSON.parse(required($('#messages').textContent)) as Record<
  string,
  string
>;
function tr(key: MessageKey, values: MessageValues = {}) {
...
const htmlTr = (key: MessageKey, values?: MessageValues) =>
  esc(tr(key, values));
const multiline = (key: MessageKey) => htmlTr(key).replace(/\n/g, '<br>');
const stellarTitle = tr('brand.title', { owner: input.owner });
const domainById = new Map(input.domains.map((d) => [d.id, d]));
const categoryById = new Map(input.categories.map((c) => [c.id, c]));
const sourceById = new Map(input.sources.map((source) => [source.id, source]));
const sourceName = (source: Source) => `${source.name} · ${source.namespace}`;
const sourceSummary = [
  ...new Set(input.sources.map((source) => source.name)),
].join(' + ');
const snapshotSummary = input.sources
...
const displayId = (issue: Pick<ViewerIssue, 'identifier' | 'sourceId'>) => {
...
const issueSource = (issue: Pick<ViewerIssue, 'sourceId'>) =>
  sourceName(required(sourceById.get(issue.sourceId)));
const issues: ViewerIssue[] = input.issues.map((issue) => {
  const category =
    issue.classification && categoryById.get(issue.classification.category);
  return {
    ...issue,
    statusType: issue.status.type,
    status:
      issue.detail === 'unqueried'
        ? tr('status.unqueried')
        : issue.status.label,
...
const active = (i: ViewerIssue | undefined | null) =>
...
const domainColor = (id: string | undefined) =>
  colors[
    Math.max(
      0,
...
const catItems = (id: string) => R.items.filter((i) => i.category === id),
  domainItems = (id: string) => R.items.filter((i) => i.domain === id);
const statusRank = {
  started: 0,
  unstarted: 1,
  backlog: 2,
...
const sortIssues = (a: ViewerIssue, b: ViewerIssue) =>
...
let base: ViewerIssue[] = [],
  baseIds = new Set<string>(),
  sceneById = new Map<string, SceneNode>(),
  initial = true,
  drag: {
    x: number;
    y: number;
  ...
  lastModalFocus: Element | null = null;
try {
...
function syncTheme() {
  ...
  $('#theme').setAttribute('aria-label', String($('#theme').title));
}
syncTheme();
...
$('#snapshot').textContent =
  (R.sources.length === 1
    ? tr('snapshot', { timestamp: required(R.sources[0]).snapshotAt })
...
$<HTMLSelectElement>('#scope').value = state.scope;
...
for (const target of [...selectableTargets].sort((a, b) =>
  ...
  $<HTMLSelectElement>('#target').add(new Option(target, target));
function scoped(i: ViewerIssue) {
  return (
    state.scope === 'all' ||
...
function updateBase() {
  ...
  $('#scope-count').textContent = String(base.length);
}
function outsideScopeKey(issue: ViewerIssue) {
...
function scopeName() {
  return required($<HTMLSelectElement>('#scope').selectedOptions[0])
    .textContent;
}
function selectedIssue() {
  return state.selected?.type === 'issue' ? all.get(state.selected.id) : null;
}
function isTarget(i: ViewerIssue | undefined | null) {
  return !state.target || i?.targets?.includes(state.target);
}
function idsIn(n: SceneNode) {
...
function targetMatch(n: SceneNode) {
  return (
    !state.target ||
    idsIn(n).some((id) => baseIds.has(id) && isTarget(required(all.get(id)))) ||
    (n.type === 'issue' && isTarget(all.get(n.key)))
  );
}
function serializeState(): Navigation {
  return {
    mode: state.mode,
...
function rememberSelection(type: NodeKind, id: string) {
...
function restorePrevious() {
  ...
  $<HTMLSelectElement>('#scope').value = state.scope;
  $<HTMLSelectElement>('#target').value = state.target;
  render(false);
  $<HTMLButtonElement>('#back').disabled = !state.history.length;
}
function closeMobileTree() {
  if (innerWidth <= 650) $('#sidebar').classList.remove('visible');
}
function selectDomain(id: string, push = true) {
...
function selectCategory(id: string, push = true) {
...
function selectIssue(id: string, push = true) {
...
function issueOffset(angle: number, count: number, index: number) {
...
function localScene(): Scene {
  const id = required(state.selected).id,
    i = required(all.get(id)),
  ...
  neighborIds.sort((a, b) => {
    const da = required(all.get(a)).domain || 'z',
      db = required(all.get(b)).domain || 'z';
    return (
      da.localeCompare(db, R.locale) ||
      a.localeCompare(b, R.locale, { numeric: true })
    );
  });
  const nodes: SceneNode[] = [makeIssueNode(id, 0, 0, true)],
    edges: SceneEdge[] = rels.map((e, j) => ({
      ...e,
      id: 'local:' + j,
      source: 'i:' + e.source,
      target: 'i:' + e.target,
...
function targetTags(i: ViewerIssue) {
  return (i.targets || [])
    .map((t) =>
...
function relationRow(otherId: string, label = '') {
  const x = required(all.get(otherId));
  return `<button class="relation-row" data-issue="${esc(otherId)}"><i class="status-dot ${esc(x.statusType)}"></i><div><b>${esc(displayId(x))}</b><span class="title">${esc(x.title)}</span><small>${esc(x.status)}${outsideScopeKey(x) ? ' · ' + htmlTr(required(outsideScopeKey(x))) : ''}</small></div>${label ? `<span class="rel-kind">${esc(label)}</span>` : ''}</button>`;
}
function summaryCounts(items: ViewerIssue[]) {
...
function renderInspector() {
  ...
  if (selected.type === 'domain' || selected.type === 'category') {
    const domain =
        selected.type === 'domain'
          ? required(domains.get(selected.id))
          : required(domains.get(required(categories.get(selected.id)).domain)),
      c =
        selected.type === 'category'
          ? required(categories.get(selected.id))
          : null;
  ...
  const i = required(all.get(selected.id)),
    rels = incident(i.id),
    byKind: Record<string, Relation[]> = Object.create(null) as Record<
      string,
      Relation[]
    >;
  for (const e of rels) {
  ...
  const parent = i.parentId ? all.get(i.parentId) : undefined,
...
function renderCaption() {
  let crumb = '',
    caption;
  if (state.mode === 'local') {
    const i = required(selectedIssue()),
      others = state.scene.nodes
    ...
    caption = `<b>${esc(required(domains.get(c.domain)).label)}</b> · ${htmlTr('caption.explore')}`;
  } else
...
function wrapLabel(text: string, maxWeight = 17, maxLines = 2) {
  const chunks = [];
  let line = '',
    w = 0;
  for (const c of text) {
...
function nodeSelected(n: SceneNode) {
  return (
    state.selected &&
...
function lineGeometry(a: SceneNode, b: SceneNode, offset = 0) {
...
function curveHitsBox(
  g: ReturnType<typeof lineGeometry>,
  box: Box,
  padding: number,
) {
  ...
  const roots = (a: number, c: number, b: number, boundary: number) => {
    const A = a - 2 * c + b,
      B = 2 * (c - a),
      C = a - boundary;
...
function boxesOverlap(a: Box, b: Box, padding: number) {
  return (
...
function placeNodeLabels(
  nodeEls: SVGGElement[],
  frame: ReturnType<typeof labelFrame> | null,
) {
  const k = state.transform.k,
    occupied: Box[] = state.scene.nodes.map((node) => {
    ...
    ordered = state.scene.nodes
      ...
      .sort((a, b) => {
        ...
        const edgeSpace = (node: SceneNode) => {
        ...
        candidates: [number, number][] = [
          ...
          ...(node.type === 'domain'
            ? ([
                ...
                ...[-32, -16, 16, 32].flatMap<[number, number]>((dx) => [
                  [dx / k, 0],
                  [dx / k, above],
                ]),
              ] satisfies [number, number][])
...
function applyTransform(geometry = false) {
  ...
  if (geometry) {
    const nodeEls = $$<SVGGElement>('#nodes .graph-node');
    state.scene.nodes.forEach((n, j) => {
      const el = required(nodeEls[j]),
        r = nodeRadius(n);
      $<SVGCircleElement>('.node-dot', el).setAttribute('r', String(r));
      $<SVGCircleElement>('.node-halo', el).setAttribute(
        'r',
        String(r + 5 / k),
      );
      $<SVGCircleElement>('.node-hit', el).setAttribute(
        'r',
        String(Math.max(r + 8 / k, 13 / k)),
      );
      const count = $<SVGTextElement>('.node-count', el);
      count.setAttribute(
        'font-size',
        String(Math.min(11, Math.max(8, r * k * 0.8)) / k),
      );
      count.textContent =
        n.type === 'domain' && n.count !== null ? String(n.count) : '';
          ...
          ? Math.min(
              ...
              Math.max(
                ...
                Math.ceil(
                  [...mainText].reduce(
                    (sum, c) =>
                      sum + (required(c.codePointAt(0)) <= 127 ? 0.53 : 1),
                    0,
                  ) / areaLines,
                ),
              ),
      ...
      if (wideArea) {
        ...
        const compact = label.cloneNode(false) as SVGTextElement,
      ...
      const bl = el.querySelector<SVGCircleElement>('.node-block');
      if (bl) {
    ...
    const edgeGroups = $$<SVGGElement>('#edges .edge-group'),
    ...
    const padding = 3 / k;
    for (const label of $$<SVGTextElement>('#edge-labels .edge-label')) {
...
function readableScale(nodes: SceneNode[]) {
  let scale = 0.015;
...
function fitScene(nodes = state.scene.nodes) {
  ...
  if (
    ...
    const frame = labelFrame(),
      ...
      clearBottom = required(frame.controls[3]).y - 8,
...
let miniBounds:
  (ReturnType<typeof bounds> & { s: number; ox: number; oy: number }) | null =
  null;
function drawMinimap() {
  ...
  $<SVGSVGElement>('#minimap').innerHTML =
    state.scene.edges
...
function updateMinimapViewport() {
  ...
  const b = miniBounds,
    ...
    wx = -t.x / t.k,
    wy = -t.y / t.k,
    el = document.querySelector<SVGRectElement>('#mini-camera');
  if (!el) return;
...
function setTarget(target: string) {
  ...
  remember();
  state.target = target;
  $<HTMLSelectElement>('#target').value = target;
  state.mode = 'global';
  state.selected = null;
  state.edgeSelection = null;
  state.openDomains = new Set(R.domains.map((d) => d.id));
...
function showSearch() {
  const q = $<HTMLInputElement>('#search').value.trim().toLocaleLowerCase(),
  ...
  panel.innerHTML =
    results
      ...
      .map(
        (i) =>
          `<button class="search-result" data-issue="${esc(i.id)}"><b>${esc(displayId(i))} · ${esc(i.status)}</b><span>${esc(i.title)}</span><small>${esc(issueSource(i))} · ${i.category ? `${esc(i.domainLabel)} › ${esc(i.group)}` : htmlTr('context.unclassified')}${outsideScopeKey(i) ? ' · ' + htmlTr(required(outsideScopeKey(i))) : ''}</small></button>`,
      )
      .join('') +
      (results.length > 30
...
function closeModal() {
  $('#modal').hidden = true;
  if (
    lastModalFocus instanceof HTMLElement ||
    lastModalFocus instanceof SVGElement
  )
...
function helpModal() {
  openModal(
    tr('help.title'),
    `<div class="help-grid"><div><h3>${htmlTr('help.explore')}</h3><ul>${(['domain', 'group', 'issue', 'history'] as const).map((k) => `<li>${htmlTr(`help.${k}`)}</li>`).join('')}</ul><p>${multiline('help.controls')}</p></div><div><h3>${htmlTr('help.connections')}</h3>${(['classification', 'direction', 'targets', 'context', 'rationale'] as const).map((k) => `<p>${htmlTr(`help.${k}`)}</p>`).join('')}<h3>${htmlTr('source.heading')}</h3>${R.sources.map((source) => `<section class="source-card"><b>${esc(sourceName(source))}</b><p>${esc(source.scope)}</p><p>${htmlTr('help.snapshot', { timestamp: source.snapshotAt })}</p><p>${htmlTr('coverage.issues')}: ${htmlTr(`coverage.${source.coverage.issues}`)} · ${htmlTr('coverage.relations')}: ${htmlTr(`coverage.${source.coverage.relations}`)}</p><p>${esc(source.notes)}</p></section>`).join('')}</div></div>`,
  );
}
function exportSVG() {
  const source = $<SVGSVGElement>('#graph'),
    clone = source.cloneNode(true) as SVGSVGElement,
    w = $('#stage').clientWidth,
    h = $('#stage').clientHeight;
  ...
  title.textContent = `${stellarTitle} · ${selectedIssue() ? displayId(required(selectedIssue())) : state.selected?.id || tr('overview')} · ${snapshotSummary}`;
  clone.prepend(title);
...
$<SVGSVGElement>('#graph').addEventListener('pointerdown', (e) => {
...
window.addEventListener('pointermove', (e) => {
  ...
  if (drag.moved) {
    $<SVGSVGElement>('#graph').classList.add('dragging');
    $('#tooltip').hidden = true;
...
window.addEventListener('pointerup', (e) => {
  ...
  drag = null;
  $<SVGSVGElement>('#graph').classList.remove('dragging');
...
$<SVGSVGElement>('#graph').addEventListener(
  'wheel',
  (e) => {
    e.preventDefault();
    $('#tooltip').hidden = true;
    if (e.shiftKey) {
      ...
      const b = $<SVGSVGElement>('#graph').getBoundingClientRect();
      zoomAt(
...
$<SVGSVGElement>('#graph').addEventListener('click', (e) => {
  if (skipClick) return;
  if (!(e.target instanceof Element)) return;
  const node = e.target.closest<HTMLElement | SVGElement>('[data-node]');
  if (node) {
  ...
  const path = e.target.closest<HTMLElement | SVGElement>('[data-edge]');
  if (path) {
    const edge = required(
...
$<SVGSVGElement>('#graph').addEventListener('pointermove', (e) => {
  if (drag?.moved) return;
  if (!(e.target instanceof Element)) return;
  const hit = e.target.closest<HTMLElement | SVGElement>('[data-node]');
  if (!hit) {
    $('#tooltip').hidden = true;
    return;
  }
  const n = required(sceneById.get(data(hit, 'node'))),
    box = $('#stage').getBoundingClientRect();
  $('#tooltip').innerHTML =
    n.type === 'issue'
      ? `<b>${esc(displayId(n.issue))} · ${esc(n.issue.status)}</b>${esc(n.subtitle)}<span>${esc(n.issue.domainLabel || tr('context.issue'))}${outsideScopeKey(n.issue) ? ' · ' + htmlTr(required(outsideScopeKey(n.issue))) : ''}</span>`
...
$<SVGSVGElement>('#minimap').addEventListener('click', (e) => {
  if (!miniBounds) return;
  const r = $<SVGSVGElement>('#minimap').getBoundingClientRect(),
...
document.addEventListener('click', (e) => {
  if (!(e.target instanceof Element)) return;
  const d = e.target.closest<HTMLElement | SVGElement>('[data-domain]');
  if (d) {
    selectDomain(data(d, 'domain'));
    return;
  }
  const c = e.target.closest<HTMLElement | SVGElement>('[data-category]');
  if (c) {
    selectCategory(data(c, 'category'));
    return;
  }
  const i = e.target.closest<HTMLElement | SVGElement>('[data-issue]');
  if (i) {
    selectIssue(data(i, 'issue'));
    return;
  }
  const t = e.target.closest<HTMLElement | SVGElement>('[data-target]');
  if (t) {
    setTarget(data(t, 'target'));
    return;
  }
  const td = e.target.closest<HTMLElement | SVGElement>(
  ...
  const tc = e.target.closest<HTMLElement | SVGElement>(
...
$<HTMLInputElement>('#search').addEventListener('input', showSearch);
...
$<HTMLSelectElement>('#scope').addEventListener('change', () => {
  remember();
  const scope = $<HTMLSelectElement>('#scope').value;
  if (!scopes.includes(scope as Scope))
    throw new Error(`Unknown scope: ${scope}`);
  state.scope = scope as Scope;
  state.edgeSelection = null;
  render(state.mode === 'global');
  showSearch();
});
$<HTMLSelectElement>('#target').addEventListener('change', () =>
  setTarget($<HTMLSelectElement>('#target').value),
);
$('#target-clear').onclick = () => setTarget('');
$$<HTMLInputElement>('[data-edge-toggle]').forEach((input) =>
  input.addEventListener('change', () => {
    const value = data(input, 'edgeToggle');
    const kind = edgeKinds.find((kind) => kind === value);
...
$<HTMLButtonElement>('#back').onclick = restorePrevious;
