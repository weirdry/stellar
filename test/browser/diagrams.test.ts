import type manifest from '../../docs/architecture/diagrams/manifest.json';
declare global {
  interface Window {
    themeEvents: number;
  }
...
void test('canonical diagrams retain their default through OS changes and preserve reader choices', async (t) => {
  ...
  for (const { name } of diagrams) {
    ...
    try {
      ...
      for (const [index, colorScheme] of (
        ['dark', 'light'] as const
      ).entries()) {
