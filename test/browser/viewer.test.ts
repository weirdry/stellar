...
for (const locale of ['ko', 'en'] as const)
  void test(
    ...
    async (t) => {
      ...
      assert.equal(must((await state()).selected).id, 'story');
      assert.equal(
      ...
      await page.mouse.move(must(box).x + 65, must(box).y + 140);
      await page.mouse.down();
      await page.mouse.move(must(box).x + 125, must(box).y + 176, { steps: 6 });
      await page.mouse.up();
      ...
      const neighbors = async (id: string) => {
        ...
        for (const e of s.edges.filter((e) =>
          ...
          assert.equal(e.source, 'i:' + must(e.actual[0]).source);
          assert.equal(e.target, 'i:' + must(e.actual[0]).target);
        }
      };
      for (const issue of data.issues.filter(
        (i: { scope: string }) => i.scope === 'assigned',
      )) {
      ...
      await select('MUS-1');
      assert.ok(
        must((await state()).nodes.find((n) => n.key === 'CTX-2')).ghost,
      );
      ...
      assert.equal(must((await state()).selected).id, 'MUS-1');
      ...
      assert.ok(
        must(await page.locator('body').getAttribute('class')).includes(
          'light',
        ),
      );
      ...
      assert.ok(
        must(await page.locator('#modal').textContent()).includes(
          must(data.sources[0]).notes,
        ),
      );
      await page.keyboard.press('Escape');
      for (const [width, height] of [
        ...
        const dims = await page.evaluate(() => {
          ...
          return {
            ...
            stage: must(document.querySelector('#stage')).clientWidth,
          };
        });
        assert.ok(
          dims.width <= must(width) &&
            dims.height <= must(height) &&
            dims.stage > 250,
          JSON.stringify(dims),
        );
        if (width === 390) {
      ...
      other.owner = attack;
      must(other.issues[0]).title = attack;
      must(other.issues[0]).status.label = '__proto__';
      await load(other);
      await select('SEED-1');
