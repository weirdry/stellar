...
function overviewMap(fixture: WorkMap, locale: WorkMap['locale']) {
  const data = structuredClone(fixture);
  data.locale = locale;
  data.view.initialScope = 'all';
  data.domains = data.domains.slice(0, 2);
  data.categories = data.categories.slice(0, 4);
  must(data.categories[2]).domain = must(data.domains[0]).id;
  const labels =
    locale === 'ko'
      ? [
          '전시 자료 해석과 관람 경험 설계',
  ...
  [...data.domains, ...data.categories].forEach((group, index) => {
    group.label = must(labels[index]);
  });
  data.issues = data.issues.slice(0, 5);
  data.issues.forEach((issue, index: number) => {
    must(issue.classification).category = must(
      data.categories[Math.min(index, 3)],
    ).id;
  });
...
async function readableLabels(page: Page) {
  const boxes = await page.locator('.graph-node').evaluateAll((nodes) =>
    nodes.flatMap((node) =>
      [...node.querySelectorAll<SVGTextElement>('.node-label tspan, .node-dot')]
        .filter(
          (element) =>
            element.closest<SVGTextElement>('.node-label')?.style.display !==
            'none',
        )
        .map((element) => {
  ...
  for (let i = 0; i < boxes.length; i++) {
    for (const b of boxes.slice(i + 1)) {
      const a = boxes[i];
      if ((!must(a).label && !b.label) || must(a).node === b.node) continue;
      assert.ok(
        must(a).right + 2 <= b.left ||
          b.right + 2 <= must(a).left ||
          must(a).bottom + 2 <= b.top ||
          b.bottom + 2 <= must(a).top,
        `labels must clear other labels and nodes: ${must(a).node} / ${b.node}`,
      );
    }
  }
  return [
...
async function clearCanvasUI(page: Page) {
  const occluded = await page.evaluate(() => {
    ...
    const controls = [
      ...
      must(document.querySelector(selector)).getBoundingClientRect(),
    );
    return [...document.querySelectorAll<SVGTextElement>('.node-label')]
      .filter((label) => label.style.display !== 'none')
      .flatMap((label) => [...label.querySelectorAll<SVGTextElement>('tspan')])
      .filter((line) => {
...
async function fittedLabels(page: Page) {
...
for (const locale of ['ko', 'en'] as const) {
  void test(`long ${locale} overview labels remain readable on a phone`, async (t) => {
    ...
    assert.ok(reduced.some((box) => must(box.node).startsWith('d:')));
    ...
    for (let i = 0; i < afterPan.length; i++) {
      assert.ok(
        Math.abs(must(afterPan[i]).left - must(beforePan[i]).left - 35) < 1,
      );
      assert.ok(
        Math.abs(must(afterPan[i]).top - must(beforePan[i]).top - 25) < 1,
      );
    }
    assert.deepEqual((await state()).nodes, initial.nodes);
    await page.locator('#fit').click();
    ...
    for (const group of [...data.domains, ...data.categories] as const) {
      const type = 'domain' in group ? 'category' : 'domain';
    ...
    assert.deepEqual(
      await page
        .locator('#data')
        .evaluate((element): unknown => JSON.parse(element.textContent)),
      data,
    );
  });
}
...
for (const locale of ['ko', 'en'] as const) {
  void test(`short ${locale} phone fit keeps area names below the canvas caption`, async (t) => {
    ...
    data.locale = locale;
    data.view.initialScope = 'all';
    data.domains.forEach((domain: { label: string }, index: number) => {
      domain.label =
        locale === 'ko'
          ? `해안 관측 기록 검토와 시민 안내 ${index + 1}`
          : `Coastal observation review and public guidance ${index + 1}`;
    });
    data.categories.forEach((category: { label: string }, index: number) => {
      category.label =
        locale === 'ko'
    ...
    data.issues.forEach((issue, index: number) => {
      must(issue.classification).category = must(data.categories[index]).id;
    });
    const ids = new Set(data.issues.map((issue) => issue.id));
    data.relations = data.relations.filter(
      (edge) => ids.has(edge.source) && ids.has(edge.target),
    ...
    const check = async () => {
      ...
      if (must(page.viewportSize()).height === 667)
        assert.ok(
          labels.some((box) => must(box.node).startsWith('d:')),
    ...
    for (let i = 0; i < afterPan.length; i++) {
      ...
      assert.ok(
        Math.abs(must(afterPan[i]).top - must(beforePan[i]).top - 20) < 1,
      );
    }
    await page.locator('#fit').click();
    await check();
