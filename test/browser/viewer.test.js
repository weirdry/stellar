...
test(
  'standalone work map: source-faithful exploration, reusable UI and safe text',
  { timeout: 120000 },
  async (t) => {
    ...
    assert.equal(await page.locator('#brand-title').textContent(), data.title);
    ...
    assert.ok(svg.includes('MUS-1') && svg.includes(data.title));
    ...
    assert.ok(
      (await page.locator('#modal').textContent()).includes(data.source.notes),
    );
    await page.keyboard.press('Escape');
    ...
    await load(other);
    assert.equal(await page.title(), other.title);
