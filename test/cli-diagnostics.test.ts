...
import { must, doctorJSON, objectJSON } from './support.ts';
...
async function setup(t: test.TestContext, complete = true) {
  ...
  const run = (
    ...
    cli = join(installed, 'bin/stellar.mjs'),
  ) =>
...
void test('version and every help topic work without schemas, resources, installer metadata, or input files', async (t) => {
...
void test('doctor diagnoses a healthy installed copy and resolves a symlink independently of cwd', async (t) => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.equal(result.stderr, '');
  const data = doctorJSON(result.stdout);
  ...
  assert.ok(
    data.checks.every((check: { status: string }) => check.status === 'pass'),
  );
...
void test('doctor reports all missing and changed resources without loading them or exposing their contents', async (t) => {
  ...
  const failed = data.checks.filter(
    (check: { status: string }) => check.status === 'fail',
  );
  assert.equal(failed.length, 6);
  assert.ok(failed.every((check) => must(check.fix).includes('Reinstall')));
  assert.match(
    must(
      failed.find(
        (check: { id: string }) => check.id === 'schemas/state.schema.json',
      ),
    ).message,
    /missing/,
  );
  assert.match(
    must(failed.find((check: { id: string }) => check.id.endsWith('en.json')))
      .message,
    /not a regular file/,
  );
...
void test('doctor treats absent or invalid manifests as failure and never follows manifest-supplied paths', async (t) => {
  const { dir, installed, work, run } = await setup(t);
  const path = join(installed, manifestPath);
  const manifest = objectJSON(await readFile(path, 'utf8'));
  ...
  for (const content of cases) {
    ...
    assert.equal(
      must(
        data.checks.find((check: { id: string }) => check.id === manifestPath),
      ).status,
      'fail',
    );
    assert.equal(
      data.checks.filter((check: { status: string }) => check.status === 'skip')
        .length,
      runtimeFiles.length,
    );
    assert.deepEqual(await snapshot(dir), before);
...
void test(
  ...
  async (t) => {
    ...
    try {
      const result = run(['doctor', '--json']);
      assert.equal(result.status, 1, result.stderr);
      const check = doctorJSON(result.stdout).checks.find(
        (item: { id: string }) => item.id.endsWith('style.css'),
      );
      assert.equal(must(check).status, 'fail');
      assert.match(must(check).message, /cannot be read/);
...
void test('doctor flags an unsupported Node runtime without hiding independent file checks', async (t) => {
  const { installed, run } = await setup(t);
  ...
  assert.equal(result.status, 1, result.stderr);
  const data = doctorJSON(result.stdout);
  assert.equal(must(data.checks[0]).status, 'fail');
  assert.match(must(must(data.checks[0]).fix), /24.x/);
  assert.ok(
    data.checks
      .slice(1)
      .every((check: { status: string }) => check.status === 'pass'),
  );
  assert.equal(run(['doctor', '--json']).status, 0);
});

void test('invalid argument counts and unknown topics fail before loading runtime or touching files', async (t) => {
...
void test('mixed help requests cannot create or overwrite outputs with valid workflow inputs', async (t) => {
  const { dir, installed, work, run } = await setup(t);
...
void test('search-issue preserves help flags as literal text and explicit relative paths remain usable', async (t) => {
  const { dir, installed, work, run } = await setup(t);
  const map = assertWorkMap(
    JSON.parse(await readFile(join(root, 'examples/museum.json'), 'utf8')),
  );
  must(map.issues[0]).description = 'Read --help or -h before starting.';
  ...
  for (const cli of [
    ...
    for (const offset of [[], ['0']]) {
      for (const flag of ['--help', '-h']) {
        const result = run(
          ['search-issue', 'map.json', must(map.issues[0]).id, flag, ...offset],
          cli,
        );
        assert.equal(result.status, 0, result.stderr);
        // Literal "-h" also occurs within "--help".
        assert.equal(objectJSON(result.stdout)['total'], flag === '-h' ? 2 : 1);
      }
    }
  }
  assert.deepEqual(await snapshot(dir), before);
...
void test('runtime load diagnostics retain a safe cause and location when doctor passes', async (t) => {
  const { installed, run } = await setup(t);
  ...
  assert.equal(doctorJSON(doctor.stdout).ok, true);
...
void test('safe error summaries omit multiline messages and forged stack frames', () => {
