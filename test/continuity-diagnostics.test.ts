import { objectJSON, array, diagnostic } from './support.ts';
...
const diagnostics = (result: SpawnSyncReturns<string>) => {
...
void test('JSON syntax diagnostics identify state, capture, choices and work-map inputs without writing', async (t) => {
  ...
  for (const command of ['refresh', 'classify', 'revise']) {
    const next =
      command === 'refresh'
        ? mixedCapture()
        : { issues: [{ issueId: must(state.map.issues[0]).id, targets: [] }] };
    for (const role of [
      'state',
      ...
      assert.equal(must(errors[0]).input, role);
      assert.equal(must(errors[0]).code, 'input-json');
      assert.equal(must(errors[0]).path, '/');
...
void test('continuity schema diagnostics point to extra fields and explain alternatives without bogus required fields', () => {
  const state = rememberMap(mixedMap());
  const typo = thrown(() => applyChoices(state, { issue: [] }, 'agent'));
  assert.ok(must(typo).some((d) => d.path === '/issue'));
  assert.ok(
    must(typo).some(
      (d) => d.path === '/' && /at least one nonempty/.test(d.message),
    ),
  );
  assert.ok(
    must(typo).every(
  ...
  assert.ok(must(escaped).some((d) => d.path === '/issue~1~0'));
  const item = thrown(() =>
    applyChoices(
      state,
      { issues: [{ issueId: must(state.map.issues[0]).id }] },
      'user',
    ),
  );
  assert.ok(
  ...
  assert.ok(must(item).every((d) => !/required property/.test(d.message)));
  ...
  assert.ok(must(badField).some((d) => d.path === '/issues/0/issueId'));
  assert.ok(
    must(badField).some(
  ...
  assert.ok(must(empty).some((d) => /nonempty/.test(d.message)));
  const badState = thrown(() =>
    assertState({ ...state, 'extra/~': 'private-probe-text' }),
  );
  assert.ok(
    must(badState).some((d) => d.path === '/extra~1~0' && d.input === 'state'),
  );
  assert.ok(must(badState).every((d) => d.fix !== must(must(typo)[0]).fix));
...
void test('null and omitted descriptions are equivalent evidence while observed facts and real text changes remain distinct', () => {
  for (const [index, field] of [
    ...
    for (const omittedFirst of [false, true]) {
      ...
      if (omittedFirst) delete must(capture.records[index]).data[field];
      else must(capture.records[index]).data[field] = null;
      const previous = rememberMap(mixedMap(capture));
      // A valid saved evidence object can contain null as well as omission.
      if (!omittedFirst)
        must(must(previous.memory[index]).evidence).description = null;
      if (omittedFirst) must(capture.records[index]).data[field] = null;
      else delete must(capture.records[index]).data[field];
      const next = refreshState(previous, capture);
      assert.deepEqual(next.changes.review, []);
      assert.deepEqual(
        must(next.map.issues[index]).classification,
        must(previous.map.issues[index]).classification,
      );
      assert.equal(
        must(next.map.issues[index]).description,
        omittedFirst ? null : undefined,
      );
      assert.equal(
        Object.hasOwn(must(next.map.issues[index]), 'description'),
        omittedFirst,
      );
      for (const text of ['', ' ', 'A new work purpose']) {
        must(capture.records[index]).data[field] = text;
        const changed = refreshState(next, capture);
        assert.ok(
          changed.changes.review.some(
            (r) =>
              r.issueId === must(next.map.issues[index]).id &&
              r.reason === 'purpose-text-changed',
          ),
        );
        assert.equal(must(changed.map.issues[index]).classification, undefined);
      }
      delete must(capture.records[index]).data[field];
      must(capture.records[index]).data.title = 'Changed purpose title';
      assert.ok(
...
void test('run output refusals are structured and preserve files, directories and symlinks', async (t) => {
  ...
  for (const output of [occupied, mapPath, alias, dangling]) {
    ...
    assert.equal(must(errors[0]).code, 'run-output');
    assert.equal(must(errors[0]).path, '/run');
    assert.match(must(errors[0]).message, /already exists/);
    assert.match(must(errors[0]).fix, /fresh unused/);
  }
  ...
  assert.match(
    must(diagnostics(run('remember', mapPath, invalidParent))[0]).message,
    /not a directory/,
  );
  if (process.getuid?.() !== 0) {
    ...
    try {
      assert.match(
        must(diagnostics(run('remember', mapPath, join(locked, 'child')))[0])
          .message,
        /not writable/,
      );
...
void test('a retained classification notice follows prior full evidence and clears only when reconsidered or fetched', () => {
  for (const actor of ['agent', 'user'] as const) {
    const map = mixedMap();
    must(must(map.issues[3]).classification).origin = actor;
    const capture = mixedCapture();
    ...
    assert.equal(must(issue).detail, 'unqueried');
    assert.equal(must(issue).status.type, 'unknown');
    assert.equal(must(must(issue).classification).origin, actor);
    assert.equal(must(issue).classificationEvidence, 'previous-observation');
    const again = refreshState(next, capture);
    assert.equal(
      must(again.map.issues.find((i) => i.id === must(issue).id))
    ...
    const targetOnly = applyChoices(
      again,
      { issues: [{ issueId: must(issue).id, targets: [] }] },
      actor,
    );
    assert.equal(
      must(targetOnly.map.issues.find((i) => i.id === must(issue).id))
        .classificationEvidence,
      'previous-observation',
    );
    const choice = {
      issues: [
        {
          ...
          classification: {
            category: must(must(issue).classification).category,
            rationale: must(must(issue).classification).rationale,
          },
        },
      ],
    };
    if (actor === 'user') {
      ...
      assert.equal(
        must(noop.map.issues.find((i) => i.id === must(issue).id))
          .classificationEvidence,
        'previous-observation',
      );
    }
    ...
    assert.equal(
      must(revised.map.issues.find((i) => i.id === must(issue).id))
        .classificationEvidence,
      undefined,
    );
    const returned = refreshState(next, mixedCapture());
    assert.equal(
      must(returned.map.issues[3]).classificationEvidence,
      undefined,
    );
    assert.ok(
    ...
    must(bad.issues[3]).classificationEvidence = 'previous-observation';
    assert.ok(
  ...
  assert.equal(
    must(fresh.map.issues.find((i) => i.id === must(context).id))
      .classificationEvidence,
    undefined,
  );
});

void test('failed output cleanup preserves the original write error and discloses only owned leftovers', async (t) => {
  ...
  const program = `
    ...
    const { mixedMap } = await import(${JSON.stringify(new URL('./fixtures.ts', import.meta.url).href)});
  ...
  const failure = objectJSON(result.stdout);
  assert.equal(failure['cause'], 'EIO');
  assert.equal(diagnostic(array(failure['diagnostics'])[0]).code, 'run-output');
  assert.match(
    must(diagnostic(array(failure['diagnostics'])[0]).cleanup),
    /may remain/,
  );
