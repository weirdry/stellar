...
1. Resolve the intended user (`get_user`, `me` only when the user means the
   ...
   Follow `hasNextPage`/`cursor` until exhausted. Do not stop at the first limit.
2. Fetch every in-scope issue with `get_issue(includeRelations: true)`. Listing
   ...
   for classification. Preserve `statusType` alongside the exact status label.
...
5. Fetch direct endpoints where accessible to explain status and context. Mark
